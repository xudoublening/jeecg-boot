package org.jeecg.modules.risk.rule.controller;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import lombok.extern.slf4j.Slf4j;
import org.jeecg.common.api.vo.Result;
import org.jeecg.common.aspect.annotation.AutoLog;
import org.jeecg.common.system.base.controller.JeecgController;
import org.jeecg.common.system.query.QueryGenerator;
import org.jeecg.modules.risk.rule.entity.Strategy;
import org.jeecg.modules.risk.rule.service.IStrategyService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.servlet.ModelAndView;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.util.Arrays;

 /**
 * @Description: 策略表
 * @Author: jeecg-boot
 * @Date:   2021-01-04
 * @Version: V1.0
 */
@Api(tags="策略表")
@RestController
@RequestMapping("/rule/strategy")
@Slf4j
public class StrategyController extends JeecgController<Strategy, IStrategyService> {
	@Autowired
	private IStrategyService strategyService;
	
	/**
	 * 分页列表查询
	 *
	 * @param strategy
	 * @param pageNo
	 * @param pageSize
	 * @param req
	 * @return
	 */
	@AutoLog(value = "策略表-分页列表查询")
	@ApiOperation(value="策略表-分页列表查询", notes="策略表-分页列表查询")
	@GetMapping(value = "/list")
	public Result<?> queryPageList(Strategy strategy,
								   @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
								   @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
								   HttpServletRequest req) {
		QueryWrapper<Strategy> queryWrapper = QueryGenerator.initQueryWrapper(strategy, req.getParameterMap());
		Page<Strategy> page = new Page<Strategy>(pageNo, pageSize);
		IPage<Strategy> pageList = strategyService.page(page, queryWrapper);
		return Result.OK(pageList);
	}
	
	/**
	 *   添加
	 *
	 * @param strategy
	 * @return
	 */
	@AutoLog(value = "策略表-添加")
	@ApiOperation(value="策略表-添加", notes="策略表-添加")
	@PostMapping(value = "/add")
	public Result<?> add(@RequestBody Strategy strategy) {
		strategyService.save(strategy);
		return Result.OK("添加成功！");
	}
	
	/**
	 *  编辑
	 *
	 * @param strategy
	 * @return
	 */
	@AutoLog(value = "策略表-编辑")
	@ApiOperation(value="策略表-编辑", notes="策略表-编辑")
	@PutMapping(value = "/edit")
	public Result<?> edit(@RequestBody Strategy strategy) {
		strategyService.updateById(strategy);
		return Result.OK("编辑成功!");
	}
	
	/**
	 *   通过id删除
	 *
	 * @param id
	 * @return
	 */
	@AutoLog(value = "策略表-通过id删除")
	@ApiOperation(value="策略表-通过id删除", notes="策略表-通过id删除")
	@DeleteMapping(value = "/delete")
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
		strategyService.removeById(id);
		return Result.OK("删除成功!");
	}
	
	/**
	 *  批量删除
	 *
	 * @param ids
	 * @return
	 */
	@AutoLog(value = "策略表-批量删除")
	@ApiOperation(value="策略表-批量删除", notes="策略表-批量删除")
	@DeleteMapping(value = "/deleteBatch")
	public Result<?> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		this.strategyService.removeByIds(Arrays.asList(ids.split(",")));
		return Result.OK("批量删除成功!");
	}
	
	/**
	 * 通过id查询
	 *
	 * @param id
	 * @return
	 */
	@AutoLog(value = "策略表-通过id查询")
	@ApiOperation(value="策略表-通过id查询", notes="策略表-通过id查询")
	@GetMapping(value = "/queryById")
	public Result<?> queryById(@RequestParam(name="id",required=true) String id) {
		Strategy strategy = strategyService.getById(id);
		if(strategy==null) {
			return Result.error("未找到对应数据");
		}
		return Result.OK(strategy);
	}

    /**
    * 导出excel
    *
    * @param request
    * @param strategy
    */
    @RequestMapping(value = "/exportXls")
    public ModelAndView exportXls(HttpServletRequest request, Strategy strategy) {
        return super.exportXls(request, strategy, Strategy.class, "策略表");
    }

    /**
      * 通过excel导入数据
    *
    * @param request
    * @param response
    * @return
    */
    @RequestMapping(value = "/importExcel", method = RequestMethod.POST)
    public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
        return super.importExcel(request, response, Strategy.class);
    }

}
